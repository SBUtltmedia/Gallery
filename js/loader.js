    ...
    $.ajax({
      ...
      .done(function(url) {
        ...
        $.ajax({
          ...
          var roomLength = Math.ceil(dict.length / 2);
          var cameraEl = document.querySelector('a-scene');
          cameraEl.setAttribute("wallblock", {
            length: roomLength
          });
          // Each art piece part is made
          dict.forEach(function(val, index) {
            ...
            var img = 'testImages-' + ((index % 6) + 1) + '.png'; //val['img'];
            var piece = val['name'];
